...
# TF-A → U-Boot → ディスク（rootdisk-7.2.img）の boot.scr → カーネル → /dev/vda2 をルートにして /sbin/init。
...
# カーネルはディスクイメージの中にある。作り方: ./make-rootdisk.sh 7.2
KERNEL_VERSION=7.2.7
DISK=rootdisk-7.2.img
...
if [ ! -f "$DISK" ]; then
    echo "$DISK がありません。先に ./make-rootdisk.sh 7.2 を実行してください" >&2
    exit 1
fi

mkdir -p logs/qemu
LOG=logs/boot-tfa-uboot-rootdisk-7.2-${CPU}.log
...
qemu-system-aarch64 \
    ...
    -nographic \
    -d guest_errors \
    -D logs/qemu/qemu-tfa-uboot-rootdisk-7.2-${CPU}.log \
    2>&1 | tee -a $LOG
