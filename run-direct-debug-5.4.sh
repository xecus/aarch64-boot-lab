...
# 別ターミナルで: gdb-multiarch -x debug-5.4.gdb
...
mkdir -p logs/qemu
LOG=logs/boot-direct-debug-5.4-${CPU}.log
echo "== kernel: linux-${KERNEL_VERSION} ==" | tee $LOG
echo "== cpu: $CPU ==" | tee -a $LOG
echo "== 別のターミナルで: gdb-multiarch -x debug-5.4.gdb ==" | tee -a $LOG
...
qemu-system-aarch64 \
    ...
    -s -S \
    -d guest_errors \
    -D logs/qemu/qemu-direct-debug-5.4-${CPU}.log \
    2>&1 | tee -a $LOG
